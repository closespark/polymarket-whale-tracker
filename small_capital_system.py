...
class SmallCapitalSystem:
    ...
    async def run(self):
        ...
        # v5: Whale management loop (promotes top performers, prunes underperformers)
        whale_management_task = asyncio.create_task(
            self.whale_management_loop()
        )
        print("🐋 Whale management loop started (hourly promotion/pruning)")
    ...
    async def whale_management_loop(self):
        """
        Periodically manage whale roster:
        1. Promote top performers from observations to active tier list
        2. Prune underperforming whales (win rate < 80%)
        3. Reload tiers into memory after changes

        Runs every hour.

        UPDATE INTERVALS (for reference):
        - Whale observation resolution: Every 60 seconds
        - Tier promotion check: Every 30 minutes
        - Whale management (this loop): Every 60 minutes
        - Position resolution: Every 30 seconds
        - Dashboard refresh: Every 5 seconds (client-side)
        """
        # Skip first run to let system initialize
        ...
        while True:
            try:
                ...
                print("\n" + "="*60)
                print("🐋 WHALE MANAGEMENT - Hourly Roster Update")
                print("="*60)

                # 1. Promote top performers from observations (min 80% win rate)
                promoted = await asyncio.to_thread(
                    db.promote_top_performers_to_tiers,
                    min_trades=5,
                    min_win_rate=0.80
                )

                # 2. Prune underperforming whales (below 80% win rate)
                pruned = await asyncio.to_thread(
                    db.prune_underperforming_whales,
                    min_win_rate=0.80,
                    min_trades=5
                )

                # 3. Reload tiers if any changes were made
                if promoted > 0 or pruned > 0:
                    print(f"   📊 Changes: +{promoted} promoted, -{pruned} pruned")

                    # Reload tiers from database into memory
                    if self.multi_tf_strategy.load_from_database(db):
                        total_whales = sum(len(tier.whales) for tier in self.multi_tf_strategy.tiers.values())
                        print(f"   ✅ Reloaded tiers: {total_whales} whales now monitored")
                    else:
                        print(f"   ⚠️ Failed to reload tiers")
                else:
                    print(f"   ℹ️ No roster changes needed")

                # 4. Log current stats
                stats = await asyncio.to_thread(db.get_whale_observations_analytics)
                if stats and stats.get('summary'):
                    s = stats['summary']
                    print(f"   📈 Observations: {s.get('unique_whales_observed', 0)} whales, "
                          f"{s.get('total_resolved_trades', 0)} resolved trades, "
                          f"{s.get('overall_win_rate', 0):.1f}% win rate")

                print("="*60 + "\n")

            except Exception as e:
                print(f"   ⚠️ Whale management error: {e}")
